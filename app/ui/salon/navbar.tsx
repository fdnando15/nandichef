  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled
          ? "bg-nandi-light/95 backdrop-blur-md shadow-lg"
          : "bg-nandi-light"
      }`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <button
            ...
            className="text-2xl font-pacifico text-nandi-primary hover:text-nandi-accent transition-colors duration-300 cursor-pointer"
          >
            Nandi Chef
          </button>
      ...
      >
        <div className="px-4 pt-2 pb-6 space-y-1 bg-nandi-light/98 backdrop-blur-md shadow-lg">
          {menuCategories.map((category) => (
            <button
              key={category.name}
              onClick={() => scrollToSection(category.href)}
